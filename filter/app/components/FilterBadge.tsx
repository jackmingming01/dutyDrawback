...
const FilterBadge: React.FC<FilterBadgeProps> = ({
    ...
    const computeBadgeLabel = (filter: FilterEntity) => {
        ...
        if (filter && typeof filter.timeRange.absoluteTimeKey === 'string') {
            isShowEditor = false;
        }
        return {label, isShowEditor};
    };
    ...
    >
        <span className={`min-w-16 max-w-44 ${isEditing ? 'text-gray-600' : ''}`}
              style={{"fontSize": ".7rem"}}>{computeBadgeLabel(filter).label}</span>
        {computeBadgeLabel(filter).isShowEditor && <button
